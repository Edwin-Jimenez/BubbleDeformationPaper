...
{\color{red} Concerning bubble breakup, Wei et
...
% {\color{red} What distinguishes our work from that in
...
$Re$ number conditions ({\color{red} $Re \ll 1$}) have been reported.}
%{\color{red} What were the $Re$ conditions considered by \cite{RusMan02, MulTobDreFisWin08} and what are the $Re$ conditions considered in our paper?}
In this work we determine, for the first time, the critical Reynolds 
number ({\color{red} $Re \gg 1$}) 
...
%{\color{red} cite some of these previous studies which examine deformation
...
details): }
\begin{description}
\item[Timestep $\Delta t$] The timestep is governed by the CFL condition and
  surface tension (section 5.7 of \cite{SUSSMAN2007469}):
 \begin{eqnarray*}
...
%% Ohta
{\color{red} Next, we examine the validation of our 
...
%%
\begin{table}[tbh]
%\captionsetup{labelfont={color=red},font={color=red}} % Not needed anymore
...
that is supported by the grid 
(see e.g. Figures \ref{fig:ShearStress} and
...
computational domain when a bubble is stretched to a $D=0.9$ Deformation,
...
For the $\Delta x^{\ell=1}=R/8$ results, it is clear 
that the $R/8$ resolution is too
coarse in order to capture the proper break-up time, albeit the critical
Reynolds' number, $Re_{\rm c}$ = 33, was still close to the 
...
for two Reynolds numbers under the condition of $Ca = 0.3$: Reynolds
number equal to 50 and 93.  
The shear stress
profile on the left corresponds to the case of $Re = 50$ and the right side
shows the shear stress profile for the case of $Re = 93$.  
The normalized shear
stress, $\tau^{\ast} = \tau / \tau_{0}$, is defined as a 
ratio of the local shear
stress $\tau = \mu_{\rm m}$$\sqrt{2\bmD:\bmD}$ 
and the apparent shear stress
$\tau_{0} = \mu_{\rm m}$$\mathit{\Gamma}$.  
In this study, for a given $Ca$ condition, the same 
value of $\tau_{0}$ is used regardless of $Re$. 
\par\noindent
For the
case of $Re = 50$, the bubble reaches a deformed stable state.  
\par\noindent
As observed in
...
We also observe that the shear stress {\em inside} the bubble was
...
break up).  On
the otherhand for Reynolds number {\em close} to the critical Reynolds
...
in close proximity to the critical Reynolds' number.  In otherwords,
