...
%% Ohta
{\color{blue} Next, we examine the validation of our computational method in which
...
deformation under the condition of $Re \approx 0$. In our study, using the
computational domain with $\lwh{12}{6}{6}$ and the finer grid sizes with 
(i)
$\Delta x^{\ell=1} = \Delta y^{\ell=1} = \Delta z^{\ell=1} = R/16$, and
(ii)
$\Delta x^{\ell=1} = \Delta y^{\ell=1} = \Delta z^{\ell=1} = R/24$, 
computations were
performed for the conditions of $Ca = 0.96$ and $Ca = 1.63$ 
with $Re \approx 0$ ($Re =
5.0 \times 10^{-4}$),  
$\lambda = 1.2 \times 10^{-3}$ and  $\eta \leq 1.2 \times 10^{-6}$.
...
{$D$}(Our study $\Delta x^{\ell=1}=R/16$)  & 0.63   &   0.76     \\
{$D$}(Our study $\Delta x^{\ell=1}=R/24$)  & 0.63   &   0.75     \\
\hline
\hline
\end{tabular}
\end{table}
