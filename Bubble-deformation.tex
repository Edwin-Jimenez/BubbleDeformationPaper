...
algorithms, and to compare with previous results, we also present simulation
results for the well-known case of drop deformation and rupture.  The numerical
...
investigations based on a Volume-of-Fluid (VOF) method \cite{HirNic81} were
...
In contrast, and to the best of our knowledge, few computational studies on
bubble deformation and breakup in shear flow have been conducted
\cite{WanShiZha15}.  This type of bubble dynamics in shear flow is also of
critical importance to further our understanding of a variety of scientific and
engineering processes.  In this article, we present computational studies of
shear-driven deformation and breakup of a bubble in insoluble viscous liquids.
The physical properties that distinguish bubble and drop studies are expressed in
terms of the density ratio $\lambda = \rho_b / \rho_m$ and the viscosity ratio
$\eta = \mu_b / \mu_m$, where $\rho$ is the fluid density, $\mu$ is the
viscosity and the subscripts ``b'' and ``m'' denote the ``bubble'' or
``drop'' and the ``matrix fluid'', respectively.  For a bubble in an insoluble
viscous liquid, the density ratio $\lambda \simeq 0$ and the viscosity ratio
$\eta \simeq 0$, whereas most studies dealing with a drop in an immiscible
...
bubble or drop in shear flow first transitions from deformation to breakup.  An
advantage of studying shear-driven bubble deformation and breakup
computationally, rather than experimentally, is that we can easily modify fluid
physical properties to ascertain the sensitivity of deformation and breakup to
physical parameters.  In our computations, the time-evolution of the boundary
between gas and liquid is tracked with a coupled level set/volume-of-fluid
(CLSVOF) interface capturing algorithm \cite{SusPuc00}.  We focus on
determining critical physical conditions in which the breakup of a bubble
occurs in shear flow because it is important to identify the parameter regimes
in which a relatively simple system transitions from stable to unstable.
Specifically, we identify the critical Reynolds number $Re_{c}$ corresponding
to the bubble breakup onset condition as a function of the Capillary number
$Ca$.  Although the $Re_{c}$ for drop breakup was reported previously
\cite{LiRenRen00}, in this work we determine, for the first time, the critical
Reynolds number that leads to \emph{bubble} breakup.  Additionally, our
computational studies reveal characteristics that distinguish the deformation
and breakup processes of a drop versus those of a bubble.
...
$\times$ $10^{-5}$ Pa$\cdot$s, and we take the density ratio $\lambda$ = 1.2
$\times$ $10^{-3}$ and the viscosity ratio $\eta$ $<$ 1.0 $\times$ $10^{-3}$.
For consistency with previous studies, we will computationally examine the
deformation and breakup of a bubble in simple linear shear flow under as a
function of $Re$ and $Ca$ numbers.  In particular, for a certain $Ca$ value, we
focus on determining the critical value $Re_{c}$ that corresponds to the threshold
for breakup.
...
\begin{figure}%[h!]
  ...
  \caption{(a) Computational domain schematic for a bubble or drop in shear 
           ...
           shear flow on the same grid system.}
...
which is based on a fixed grid finite difference algorithm.  In our studies,
two-phase fluid flow is comprised of air and a viscous Newtonian liquid. A
single set of three-dimensional equations governs the motion of both fluids,
which are taken to be incompressible, and consists of the continuity equation
and the Navier-Stokes equations with surface tension forces:
%
\begin{align}
...
is the viscosity, $\kappa$ is the interfacial curvature, and $\Hea(\phi)$ is
the Heaviside function as a function of the level set (LS) function $\phi$. The
singular term (last term) in the right hand side of equation~\eqref{nseqn} is a
body force due to surface tension and is equivalent to specifying that the jump
in the normal stress is equal to $\sigma\kappa$.  The interfacial curvature
$\kappa$ is computed with second order accuracy directly from the
volume-of-fluid (VOF) function using the height function technique.  We remark
that we get the same results if we were to compute $\kappa$ directly from the
LS function using the ``level set'' height function technique.
...
\end{eqnarray}
%
The subscripts ``b'' and ``m'' refer to ``drop or bubble'' and ``matrix fluid'',
respectively.  The Heaviside function, $\Hea(\phi)$, is defined as follows:
%
...
\end{equation}
%
To represent the free surface with the CLSVOF method, we must evolve the solution
to both the VOF equation for $F$ and the LS equation for $\phi$:
%
...
dynamically adjusted based on the location of a deforming gas-liquid interface.
...
\subsection{Validation of numerical method}
%  -----------------------------------------------------------------------------
Numerical studies are presented to verify the accuracy of our computational
methods.  First, we compare quantitatively against the steady-state drop
...
%
\begin{table}[tbh]
\caption{Comparison of the Taylor deformation parameter $De$ as a function 
         of $Re$ ($Ca=0.3$, $\lambda = \eta = 1$).}
...
\begin{figure}[h!]
  ...
  \caption{Comparison with results reported in~\cite{RenCri01-2} (shown in boxes) 
           for drop breakup in shear flow. (Reprinted with permission from 
           reference~\cite{RenCri01-2}. Copyright 2001, AIP Publishing). Results
           obtained with our CLSVOF algorithm are shown outside boxes. The onset
           of drop breakup is demonstrated for (a) $Re=10$, $Ca=0.16$,
           (b) $Re=12$, $Ca=0.1753$, and (c) $Re=15$, $Ca=0.196$.}
  \label{fig:DropBreakComp}
\end{figure}
%

Next, we present a comparison with numerical results for drop breakup reported
in~\cite{RenCri01-2}.  Figure~\ref{fig:DropBreakComp} demonstrates drop
breakup with pinch-off behavior for three $Re$ and $Ca$ conditions and with
constant values of $\lambda = \eta = 1$ for all cases.  The three cases that we
consider correspond to (a) $Re = 10, \, Ca = 0.16$, (b) $Re = 12, \, Ca =
0.1753$, and (c) $Re = 15, \, Ca = 0.196$, and which are illustrated in
...
(level-0) discretization; we set the finer level-1 grid size $\Delta x^{\ell=1}
...
used our study in the level-1 grid around the elongated drop.  The numerical
...
%
\begin{table}[tbh]
\caption{Comparison of the Taylor deformation parameter $De$ as a function of
...
\begin{figure}%[h!]
  ...
  \caption{Time evolution of the deformation parameter $De$ versus a dimensionless
           ...
           = R/24$, and they predict that breakup occurs at $Re=34$.}
...
x^{\ell=1}=R/24$ predicts a critical value of $Re_c = 34$. Although there is
only a relatively small difference ($\sim 3\%$) between the critical Reynolds
numbers, and the time evolution of $De$ for the two grid systems is consistent
in both cases, the computational time using the grid system with
$\Delta x^{\ell=1}=R/24$ was more than 6 times longer than the one based on the
coarser system with $\Delta x^{\ell=1}=R/16$.  We note that in our search for
$Re_c$, we considered a wide range of values of $Ca$ and we found it necessary
to use a large $L$ ($\sim 24R$) since for certain shear flows the bubble can
stretch significantly without breaking up.  Nevertheless, for the conditions
presented in this section, the results indicate that our numerical approach,
even with a finest-level resolution set to $\Delta x^{\ell=1}=R/16$, is capable of
...
\begin{figure}%[h!]
  ...
  \caption{Time evolution of drop deformation and breakup in shear flow at the
           condition of $Ca=0.3$ with (a) $Re=1.0$ and (b) $Re=1.1$.}
  \label{fig:DropBreak}
\end{figure}
%
...
55.2$, a thread-bridge forms between the bulbar ends and the thread-bridge
...
magnitude is required for bubble breakup ($\lambda$ = $\eta$ = 1.0 $\times$
$10^{-3}$) compared with the case of the drop ($\lambda$ = $\eta$ = 1). Then,
...
case of the drop with $\lambda$ = $\eta$ = 1 (see e.g.~\cite{LiRenRen00}).
...
Table~\ref{tab:CaRecComparison} lists columns of Capillary number and critical
Reynolds number pairs for shear-induced bubble breakup for a variety of flow
...
effects of very small density and viscosity ratios ($\lambda$ = $\eta$ = 1.0
$\times$ $10^{-3}$) act synergistically in deformation and breakup processes.
%
\begin{table}[tbh]
\caption{Critical Reynolds number corresponding to Capillary number}
