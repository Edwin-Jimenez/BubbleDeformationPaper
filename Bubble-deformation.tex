...
undergoing simple linear shear flow are presented.  Numerical results are
...
interface-capturing} level set methods \cite{SusSmeOsh94}.  {\color{red} In
contrast, only a few computational studies on bubble deformation and breakup in
shear flow can be found \cite{WeiQiaXu12,WanShiZha15}.  In these previous
studies, the dynamics of bubble deformation in shear flow has mainly been
examined.} {\color{blue} Concerning bubble breakup, Wei et
...
This type of bubble dynamics in shear flow is of critical importance for a
...
we present computational studies of shear-driven deformation and breakup of a
...
in a simple shear flow \cite{RusMan02, MulTobDreFisWin08}, only findings for bubble defomation under very low 
$Re$ number conditions ({\color{blue} $Re \ll 1$}) have been reported.}
%{\color{blue} What were the $Re$ conditions considered by \cite{RusMan02, MulTobDreFisWin08} and what are the $Re$ conditions considered in our paper?}
...
1$\cite{?}.  Thus, for comparison with previous drop deformation and breakup
...
%% Ohta
{\color{blue} Next, we examine the validation of our computational method
intended for the bubble deformation in simple linear shear flow reported by
...
deformation under the condition of $Re \approx 0$. In this study, using the
computational domain with $\lwh{12}{6}{6}$ and the finer grid size with $\Delta
x^{\ell=1} = \Delta y^{\ell=1} = \Delta z^{\ell=1} = R/16$, computations were
performed for the condition of $Ca = 0.96$ and  1.63 with $Re \approx 0$ ($Re =
5.0 \times 10^{-4}$),  $\lambda = 1.2 \times 10^{-3}$ and  $\eta \leq 1.2 \times 10^{-6}$
according to experimental conditions by Muller-Fischer et al.
\cite{MulTobDreFisWin08}.  Comparison of our numerical results and previous
...
\caption{Comparison of  {\color{red} $D$} for a bubble as a function 
         of $Ca$ ($Re \approx 0$, $\lambda \approx \eta \approx 0$). In all cases, the domain 
         size is $\lwh{12}{6}{6}$.
...
{\color{red} The numerical validation studies performed in this section and the
...
we refer the reader to \cite{ohta2011robust} in which we apply our CLSVOF
...
\begin{figure}%[h!]
  ...
  \caption{Normalized shear stress $\tau^{\ast}$ around a bubble for two Reynolds 
...
% Yes. I have been suspending computations for bubbles because I have been focussing on non-Newtonian cases, but I have a plan for restarting again.
